...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([item[@"type"] isEqualToString:@"folder"]) {
        ...
        [self showAudioInfoForItem:item];
    }
}

- (NSString *)codecNameForFormatID:(AudioFormatID)formatID {
    switch (formatID) {
        ...
        default: {
            char cc[5] = {(char)((formatID>>24)&0xFF),(char)((formatID>>16)&0xFF),(char)((formatID>>8)&0xFF),(char)(formatID&0xFF),0};
            return [NSString stringWithFormat:@"%s", cc];
        }
    }
}

- (void)showAudioInfoForItem:(NSDictionary *)item {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        if (secs >= 60)
            [info appendFormat:@"时长：%d:%02d\n", (int)(secs/60), (int)secs%60];
        else if (secs > 0)
            [info appendFormat:@"时长：%.1f 秒\n", secs];
        ...
        if (track) {
            ...
            if (desc) {
                ...
                if (asbd) {
                    // 编码格式
                    [info appendFormat:@"编码：%@\n", [self codecNameForFormatID:asbd->mFormatID]];
                    // 采样率
                    if (asbd->mSampleRate > 0)
                        [info appendFormat:@"采样率：%.0f Hz\n", asbd->mSampleRate];
                    // 声道
                    NSString *ch = asbd->mChannelsPerFrame == 1 ? @"单声道" : asbd->mChannelsPerFrame == 2 ? @"立体声" : [NSString stringWithFormat:@"%u 声道", asbd->mChannelsPerFrame];
                    [info appendFormat:@"声道：%@\n", ch];
                    // 位深（PCM 才有意义）
                    if (asbd->mFormatID == kAudioFormatLinearPCM && asbd->mBitsPerChannel > 0)
                        [info appendFormat:@"位深：%u bit\n", asbd->mBitsPerChannel];
                }
            }
            // 码率
            float bps = track.estimatedDataRate;
            if (bps > 0)
                [info appendFormat:@"码率：%.0f kbps", bps / 1000.0];
        }

        dispatch_async(dispatch_get_main_queue(), ^{
            UIAlertController *alert = [UIAlertController alertControllerWithTitle:item[@"name"] message:info preferredStyle:UIAlertControllerStyleAlert];
